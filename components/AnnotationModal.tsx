...
export const AnnotationModal: React.FC<AnnotationModalProps> = ({ 
  ...
  }, [isOpen, initialComment, initialSeverity]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-start" style={{ pointerEvents: 'none' }}>
      ...
      <div 
        className="absolute bg-white rounded-lg shadow-xl border border-gray-200 p-4 w-80 flex flex-col gap-3 animate-in fade-in zoom-in-95 duration-200"
      ...
      >
        ...
        <div className="flex flex-col gap-2">
          <textarea
            ref={inputRef}
            className="w-full h-24 p-2 border border-gray-300 rounded-md resize-none text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter' && !e.shiftKey) {
                e.preventDefault();
                if(comment.trim()) onSubmit(comment, severity);
              }
            }}
          />
          <select 
